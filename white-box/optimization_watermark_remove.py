...
class WatermarkDetectorWrapper():
    def __init__(self, model, message, watermark_message, on_bitstring, model_type, threshold, device):
        self.model = model
        self._device = device
        self.message = message.to(self._device)
        self.watermark_message = watermark_message.to(self._device)
    ...
    def bwacc_audioseal(self, signal):
        ...
        if self.on_bitstring:
            if msg_decoded is None:
                ...
                bitacc = 1 - torch.sum(torch.abs(self.watermark_message - msg_decoded)) / self.watermark_message.numel()
                return bitacc
        else:
            return result
    ...
    def bwacc_timbre(self, signal):  # signal is tensor on gpu
        ...
        target_message = self.watermark_message.detach()
    ...
    def optimize_loss_audioseal(self, original_watermark_audio, new_watermark_audio, gt_message_input):
        ...
        gt_message_input = torch.full_like(gt_message_input, 0.5)
        ...
        with torch.no_grad():
            ...
            if snr > best_snr and detector.bwacc(new_watermark_audio) <= args.tau:
                ...
                for index in range(0, len(noise_message)):
                    if noise_message[index] > left and noise_message[index] < right:
                        cnt -= 1
                    print("cnt: ", cnt)
                    if cnt <= 0:
                        best_snr = snr
...
def decode_audio_files_perturb_whitebox(model, output_dir, args, device):
    ...
    for file_num, watermarked_file in progress_bar:
        ...
        if args.dataset == 'librispeech':
            ...
            idx = os.path.splitext(base_name)[0]
            watermarked_file_new = next((f for f in adv_signal_files if f.startswith(base_no_ext + "_")), None)
            waveform, sample_rate = torchaudio.load(os.path.join(output_dir, 'watermark_timbre_audiomark_16khz', watermarked_file))
            ...
            idx = os.path.splitext(base_name)[0]
            watermarked_file_new = next((f for f in adv_signal_files if f.startswith(base_no_ext + "_")), None)
            waveform, sample_rate = torchaudio.load(os.path.join(output_dir, 'watermark_timbre_gigaspeech_xs', watermarked_file))
        ...
        if args.model == "audioseal":
            _ , watermark_message = model.detect_watermark(waveform)
            ...
            watermark_message = model.test_forward(waveform).squeeze()
        ...
        detector = WatermarkDetectorWrapper(model, attacked_message, watermark_message, attack_bitstring, args.model, args.tau, device)
        if args.model == "audioseal":
