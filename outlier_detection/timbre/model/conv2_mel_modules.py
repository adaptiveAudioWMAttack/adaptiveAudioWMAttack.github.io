...
def get_vocoder(device):
    with open("white-box/timbre/hifigan/config.json", "r") as f:
        config = json.load(f)
    config = timbre.hifigan.AttrDict(config)
    vocoder = timbre.hifigan.Generator(config)
    ckpt = torch.load("white-box/timbre/hifigan/model/VCTK_V1/generator_v1")
